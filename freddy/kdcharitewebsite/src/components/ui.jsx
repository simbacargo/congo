...
// <g> for the gradient highlight, <e>/<eb> for red emphasis, <em> for the
...
const RICH_COMPONENTS = {
  ...
  e: <span className="text-red-400 font-semibold" />,
  eb: <span className="text-red-400 font-bold" />,
  em: <em className="text-red-400 not-italic font-semibold" />,
  br: <br />,
};
...
export function Badge({ icon: Icon, children, tone = 'red' }) {
  const tones = {
    red: 'glass-red text-red-400',
...
export function CrossCTA({
  ...
  return (
    <section className="py-20 px-4">
      <Reveal className="max-w-4xl mx-auto glass-red rounded-3xl p-8 md:p-12 border-glow text-center">
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          {secondary && (
            <Link
              to={secondary.to}
              className="flex items-center gap-2 glass hover:border-red-500/40 text-white font-semibold px-6 py-3 rounded-2xl transition-all hover:-translate-y-0.5"
            >
              {secondaryLabel}
            </Link>
          )}
