...
function LiveCounters() {
  ...
  const stats = [
    { label: t('hero.stats.fundsLabel'), value: funds, icon: DollarSign, color: 'text-red-400', glow: 'rgba(239,68,68,0.25)', sub: t('hero.stats.fundsSub'), fmt: (n) => '$' + Math.round(n).toLocaleString() },
    ...
    { label: t('hero.stats.stationsLabel'), value: stations, icon: Fuel, color: 'text-red-300', glow: 'rgba(252,165,165,0.2)', sub: t('hero.stats.stationsSub'), fmt: (n) => Math.round(n).toLocaleString() },
  ];
    ...
    >
      {stats.map((s) => (
        <motion.div
          ...
          variants={fadeUp}
          whileHover={{ y: -4 }}
          className="glass rounded-2xl p-6 group hover:border-red-500/30 transition-colors duration-300 relative overflow-hidden"
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-red-400 font-medium uppercase tracking-widest">{t('hero.live')}</span>
            </div>
          </div>
...
export default function Hero() {
  ...
  return (
    <section ref={scope} id="top" className="relative min-h-screen flex flex-col justify-center px-4 overflow-hidden">
      ...
      <div data-orb className="absolute top-1/4 -left-40 w-96 h-96 bg-red-600/15 rounded-full blur-[100px]" />
      <div data-orb className="absolute bottom-1/4 -right-40 w-80 h-80 bg-amber-500/10 rounded-full blur-[90px]" />
      <div data-orb className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] border border-red-500/[0.05] rounded-full animate-spin-slow pointer-events-none" />
      <div data-orb className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] border border-red-500/[0.08] rounded-full animate-spin-slow pointer-events-none" style={{ animationDuration: '14s', animationDirection: 'reverse' }} />
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <motion.div variants={container} initial="hidden" animate="show">
            <motion.div variants={fadeUp} className="flex items-center gap-3 mb-8">
              <span className="glass-red rounded-full px-4 py-1.5 text-xs font-semibold text-red-400 uppercase tracking-widest">
                {t('hero.badge')}
              </span>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-4 mb-14">
              <MagneticButton
                to="/donate"
                className="group flex items-center gap-2.5 bg-red-600 hover:bg-red-500 text-white font-bold px-7 py-3.5 rounded-2xl shadow-xl shadow-red-900/50 cursor-pointer"
              >
              ...
              <Link
                to="/how-it-works"
                className="flex items-center gap-2.5 glass hover:border-red-500/40 text-white font-semibold px-7 py-3.5 rounded-2xl transition-all duration-200 hover:-translate-y-0.5"
              >
                {t('hero.howItWorks')}
                <ChevronDown size={15} />
              </Link>
                ...
                <div>
                  ...
                  <p className="text-xs text-white/45 leading-relaxed">
                    <Trans
                      ...
                      components={{ e: <span className="text-red-400 font-semibold" /> }}
                    />
                  </p>
                </div>
              </div>
