...
<section class="card">
  ...
  {#if loading}<Loading />{:else if error}<ErrorState message={error} retry={load} />{:else if rows.length}<div class="table-wrap"><table class="data-table"><thead><tr>{#if can("bulk_update_transactions")}<th></th>{/if}<th>Receipt</th><th>Date</th><th>Company / Station</th><th>Church</th><th>Agent / Fuel</th><th class="right">Amount</th><th class="right">Levy</th><th>Status</th></tr></thead><tbody>{#each rows as tx}<tr>{#if can("bulk_update_transactions")}<td><input type="checkbox" checked={selected.has(tx.id)} on:change={() => toggle(tx.id)} /></td>{/if}<td><a class="text-link receipt-code" href={routePath("transaction-detail", tx.id)} on:click={(event) => { event.preventDefault(); navigate(routePath("transaction-detail", tx.id)); }}>{tx.receipt_code}</a></td><td class="nowrap">{dateTime(tx.created_at)}</td><td><strong>{tx.station_name}</strong><small class="subtext">{tx.company_name}</small></td><td>{tx.church_name}</td><td><strong>{tx.agent_username}</strong><small class="subtext">{tx.fuel_type_name}</small></td><td class="right">{money(tx.amount_usd)}</td><td class="right money">{money(tx.levy_amount_usd)}</td><td><StatusBadge status={tx.status} /></td></tr>{/each}</tbody></table></div><Pagination count={total} page={page} pageSize={pageSize} onPage={(next) => { page = next; load(); }} />{:else}<EmptyState message="No transactions match these filters." />{/if}
</section>
